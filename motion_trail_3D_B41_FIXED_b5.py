...
class IndirectChannel(object):
    ...
    def _get_fcurves(self, action):
        """Get fcurves from action, supporting both Blender 4.x and 5.0+ APIs"""
        # Try new Blender 5.0 API first
        if hasattr(action, 'slots') and len(action.slots) > 0:
            try:
                ...
                pass
        # Fall back to legacy API (Blender 4.x)
        if hasattr(action, 'fcurves'):
            return action.fcurves
        return None
    
    def exists(self):
        if self.action_name not in bpy.data.actions:
            return False
        action = bpy.data.actions[self.action_name]
        fcurves = self._get_fcurves(action)
        if fcurves is None:
            return False
        return fcurves.find(self.data_path,index=self.array_index) != None
    ...
    def deref(self):
        ...
        if fcurves is None:
            return None
        return fcurves.find(self.data_path,index=self.array_index)
...
class POSE_OT_Flip_Quaternion_Ipo_Direction(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # Get fcurves supporting both Blender 4.x and 5.0+
        fcurves = None
        if hasattr(action, 'slots') and len(action.slots) > 0:
            try:
                channelbag = anim_utils.action_get_channelbag_for_slot(action, action.slots[0])
                ...
                pass
        if fcurves is None and hasattr(action, 'fcurves'):
        ...
        if context.mode == 'POSE':
            ...
            channels = [fcurves.find(data_path,index=i) for i in range(0,4) ] if fcurves else [None] * 4
        else:
            channels = [fcurves.find('rotation_quaternion',index=i) for i in range(0,4)] if fcurves else [None] * 4
